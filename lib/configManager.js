/**
 * ConfigManager - Persistent config storage for deep-research
 * 
 * Stores API keys and settings in a platform-standard location:
 *   Windows:  %APPDATA%/deep-research/config.json
 *   macOS:    ~/Library/Application Support/deep-research/config.json
 *   Linux:    ~/.config/deep-research/config.json
 * 
...
function getConfigDir() {
  ...
  if (platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "deep-research");
  } else if (platform === "darwin") {
    return path.join(os.homedir(), "Library", "Application Support", "deep-research");
  } else {
    return path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"), "deep-research");
  }
}
